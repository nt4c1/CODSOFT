...
const UserList = ({ setTeam, team }) => {
  ...
  useEffect(() => {
    if (team?.length > 0) {
      ...
      setSelectedUsers([data[0]]); // Default to the first user if team is empty
    }
  }, [team, data]);
      ...
      <Listbox value={selectedUsers} onChange={handleChange} multiple>
        <div className='relative mt-1'>
          <Listbox.Button className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
            <span className='block truncate'>{selectedUserNames}</span>
          ...
          >
            <Listbox.Options className='z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
              {data?.length > 0 ? (
                data.map((user) => (
                  <Listbox.Option
                    key={user._id} // Use a unique key from user object
                    className={({ active }) =>
                ...
                <div className="py-2 text-center text-gray-500">No users available</div>
              )}
            </Listbox.Options>
          </Transition>
        </div>
